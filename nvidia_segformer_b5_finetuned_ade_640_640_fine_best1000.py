# -*- coding: utf-8 -*-
"""nvidia/segformer-b5-finetuned-ade-640-640_Fine_best1000.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1BAePBPON7pGBfZYJRHT1_svC3c1S4yHq
"""
...
# 모든 이미지 파일 경로 불러오기 (JPEG, JPG, PNG 파일 포함, 대소문자 구분 없음)
...
# 이미지 파일들을 하나의 리스트로 결합
all_image_paths = sorted(all_image_paths_jpeg + all_image_paths_jpg + all_image_paths_png)

# 파일 이름에 따라 train 셋과 val 셋으로 나누기
train_image_paths = [path for path in all_image_paths if "ADE_train" in path]
val_image_paths = [path for path in all_image_paths if "ADE_val" in path]

# 라벨 파일 경로 설정 (PNG 파일, 대소문자 구분 없음)
all_label_paths = sorted(glob("/content/drive/MyDrive/water_v/water_v2/water_v2/Annotations/**/*.[pP][nN][gG]", recursive=True))

# 파일 이름에 따라 train 라벨과 val 라벨로 나누기
...
# 데이터셋 로드
train_dataset = CustomSegmentationDataset(train_image_paths, train_label_paths, mean, std)
val_dataset = CustomSegmentationDataset(val_image_paths, val_label_paths, mean, std)
...
train_model(num_epochs=200, patience=5, warmup_steps=100, max_grad_norm=0.8, save_path='best_model.pth')
...
# 모델 훈련 시작
train_model(num_epochs=100, patience=5, warmup_steps=100, max_grad_norm=0.8, save_path='best_model.pth')
...
def evaluate_on_test_set():
    ...
    # 결과 출력
    print(f"Test Loss: {avg_test_loss:.4f}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1_score:.4f}, IoU: {iou:.4f}, Dice: {dice:.4f}")
...
model_save_path = '/content/drive/MyDrive/nvidia_best.pth'
